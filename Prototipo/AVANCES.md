...
- **Si la numeración automática debe incluir el municipio.** El apunte de la Dirección decía que el
  ...
  SIMH sigue siendo general (`SFS/SMH/####/2026` desde el 23/09/2026, como en las plantillas) y que
  el municipio captura la suya aparte, en el campo
  *folio de la dependencia*. Si lo que se quería es una serie por municipio (`SFS/SMH/TAP/####/2026`),
...
- **El documento del 15/09/2026 pide quitar las tachaduras del historial telefónico, y eso contradice
  ...
  **Resuelto el 23/09/2026:** la Dirección pidió no mostrar el número tachado. Se aplicó como cambio
  de presentación: la vista muestra solo el contacto vigente y el historial sigue en los datos (§6, 24).
...
| 24 | 23/09/2026 | `assets/js/simh.js`, `assets/css/simh.css`, `assets/js/solicitud-empleo.js`, `canalizaciones.html`, `salud.html`, `revalidacion.html`, `capacitacion.html`, `constancias.html`, `registro.html`, `familiar.html`, `expedientes.html`, `expediente-detalle.html`, `empleabilidad.html`, `index.html`, `administracion.html` | Antes de presentar: (1) vistas previas y exportación de PDF consistentes en todos los módulos, con botón «Ver vista previa» a pantalla completa como la del aviso de privacidad y «Imprimir o guardar PDF» donde no lo había; (2) fotos ficticias para ver el hover y la constancia de registro; (3) folios de oficio como los de «Recursos y plantillas»; (4) no mostrar el número anterior (tachado) en Expedientes › Contacto. | **Aplicado** | (1) Un solo visor, `SIMH.verDocumento(html, título, opts)`, y una sola botonera, `SIMH.barraDoc(título)` («Vista previa» · Ver vista previa · Imprimir o guardar PDF), usados por Canalizaciones, Salud, Revalidación, Capacitación (simulador de oficios), Solicitud SNE y el aviso de privacidad, que ahora usa el visor común. Clic sobre cualquier hoja abre el mismo visor. Las exportaciones «PDF» que no hacían nada (Panel, Expedientes, Reporte de vinculación laboral, Lista para firma de Capacitación, Constancia del Expediente Familiar, constancia de inscripción de Capacitación que imprimía la pantalla entera) salen ahora en hoja oficial con `SIMH.hojaReporte`. En Constancias el visor imprime pasando por la emisión registrada («Emitir e imprimir»), porque cada impresión es un hecho; se quitó el «Descargar PDF» de Canalizaciones, que solo mostraba una alerta y duplicaba la botonera. (2) `SIMH.fotoPersona` / `SIMH.retrato`: retratos ilustrados en SVG, generados del folio, en listados, cabeceras, cajones selectores, el expediente, la constancia (gafete y carta) y la solicitud SNE; ampliación flotante al pasar el cursor **y al enfocar con teclado**. 0410 y 0411 siguen sin foto a propósito (casos de revisión). De paso: en gafete la vigencia se encimaba con el texto legal; «Capturó» quedó solo en carta. (3) La serie pasó de `SFS/####/2026` a `SFS/SMH/####/2026`, el formato «Oficio No. SFS/SMH/0008/2026» de las plantillas; Salud llevaba un contador local y ahora usa `SIMH.folioOficio()`. (4) El contacto muestra solo lo vigente; los números anteriores se conservan en los datos (RF03), no en la vista. Resuelve la decisión abierta de §5.3. Verificado con capturas de cada visor abierto. |
| 25 | 23/09/2026 | `assets/js/simh.js`, `canalizaciones.html`, `salud.html`, `revalidacion.html`, `capacitacion.html` | En los borradores, «Oficio No. se asigna al firmar» debe verse como en la plantilla. | **Aplicado** | El borrador muestra `Oficio No. SFS/SMH/____/2026`: el formato de la plantilla con el consecutivo en blanco (resaltado), que se llena al firmar. No se muestra el siguiente número por adelantado porque varios borradores abiertos enseñarían el mismo folio. Se centralizó en `hojaSFS` (los cuatro módulos dejaron de pasar su propio texto). Aclarado también que «Oficios del grupo · simulador» de Capacitación viene del punto 3.2 del plan del 22/09/2026. |
| 26 | 23/09/2026 | `capacitacion.html`, `expediente-detalle.html`, `expedientes.html`, `assets/js/solicitud-empleo.js`, `empleabilidad.html`, `assets/css/simh.css` | Quitar la tarjeta «Oficios del grupo · simulador» de Capacitación; que la foto de la Solicitud de Empleo sea opcional; quitar las notas al pie de Contacto y del listado de Expedientes. | **Aplicado** | Se retiró el simulador completo (tarjeta, redacción, firma simulada y su CSS); el punto 3.2 del plan del 22/09/2026 queda descartado por la Dirección. En la Solicitud SNE, bloque «Fotografía (opcional)» al inicio del formulario: usar la del expediente, agregar o cambiar una propia (archivo de imagen) o quitarla y dejar la casilla en blanco, como el formato impreso. La elección se guarda con la solicitud (`datos.foto`). |
| 27 | 24/09/2026 | `salud.html` | En «Canalización al sector salud», agregar como institución el destinatario de la plantilla («A las personas titulares de los centros de salud del municipio de Tapachula, Chiapas») y quitar la nota «El oficio se genera en borrador y queda pendiente de la firma…». | **Aplicado** | La opción va primera en «Institución receptora» y dibuja el destinatario literal de la plantilla (siempre Tapachula, aunque la persona sea de otro municipio). Se retiró el subtítulo de la tarjeta. |
| 28 | 24/09/2026 | `salud.html` | En «Canalización al sector salud», precargar el asunto «Solicitud de atención a personas en contexto de movilidad» y poder editarlo en la vista previa (se preguntó si equivalía a «Motivo»). | **Aplicado** | «Motivo» no es el asunto: no se imprime, se guarda en la tabla de canalizaciones y lo precargan los marcadores, así que se conserva. Se agregó el campo «Asunto del oficio» (precargado con el de la plantilla, editable) que actualiza la hoja en vivo; sin editar conserva el salto de línea de la plantilla. Se guarda con la canalización (`asunto`). |
| 29 | 24/09/2026 | `index.html` | En Inicio, «Atención por municipio» debe llevar un mapa que se vea profesional. | **Aplicado** | El polígono esquemático se sustituyó por un mapa de Chiapas trazado con coordenadas geográficas aproximadas: costa del Pacífico, frontera con Guatemala (discontinua), estados vecinos y los seis municipios en su ubicación real con círculos proporcionales en área (leyenda de tamaños, escala de 100 km y norte). Pasar el cursor o enfocar muestra la cifra y el %; clic o Enter filtra todo el panel por ese municipio (otra vez lo quita). La tabla pasó a «Ver tabla». Mismo día, a pedido («que se vea como el mapa real»): el trazo a mano se sustituyó por límites reales — los 124 municipios del Marco Geoestadístico INEGI 2022 (vía github.com/PhantomInsights/mexico-geojson), países de Natural Earth 10m y estados vecinos — en `assets/js/mapa-chiapas.js` (generado, ~112 KB, no editar a mano). Los seis municipios de captura llevan su polígono sombreado; la fuente se cita bajo el mapa. |
| 30 | 24/09/2026 | `salud.html`, `registro.html`, `canalizaciones.html` | Que no quede ninguna nota del requerimiento RF07 en el sistema. | **Aplicado** | Se quitó «(RF07)» del subtítulo de Salud y Vulnerabilidad, de la nota de marcadores y del atajo «Valorar salud y vulnerabilidad» del alta, y «· RF07» del fundamento de la plantilla de canalización a salud. Ninguna pantalla muestra ya «RF07». |
---
